...
public class KafkaUtil {
     ...
     */
    public static List<ConsumerRecord<byte[], byte[]>> withDrawNumberedRecordsFromTopic(
            String topic, Properties properties,int number,boolean committed) throws KafkaException {
        ...
        try (KafkaConsumer<byte[], byte[]> consumer = new KafkaConsumer<>(consumerConfig)) {
            ...
            while (!topicPartitions.isEmpty()) {
                ...
                for (ConsumerRecord<byte[], byte[]> r : records) {
                    ...
                    if(consumerRecords.size()>number)
